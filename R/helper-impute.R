...
impute_date_day <- function(year, month) {
    ...
    date_1_month <- sprintf("%s-%s-01", year, month)
    ...
    at_upper_limit <- offset == days_in_month  ## unlikely, but check anyway
...
impute_date_month <- function(year, day) {
    ...
    n <- length(year)
    is_leap_year <- is_leap_year(year)
    not_feb <- (day > 29L) | (!is_leap_year & (day == 29L))
    not_feb_apr_jun_sep <- day == 31L
    i_month_max <- rep(12L, times = n)
    i_month_max[not_feb] <- 11L
    i_month_max[not_feb_apr_jun_sep] <- 7L
    i_month <- as.integer(stats::runif(n) * i_month_max) + 1L
    over_limit <- i_month > i_month_max ## unlikely, but check anyway
...
impute_date_month_day <- function(year) {
    ...
    n <- length(year)
    is_leap_year <- is_leap_year(year)
    days_in_year <- ifelse(is_leap_year, 366L, 365L)
    date_1_jan <- sprintf("%s-01-01", year)
    ...
    at_upper_limit <- offset == days_in_year  ## unlikely, but check anyway
    offset[at_upper_limit] <- offset[at_upper_limit] - 1L
