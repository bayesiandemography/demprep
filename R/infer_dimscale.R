...
infer_dimscale_period <- function(labels) {
    ...
    has_na <- any(is_na)
    n_mid <- sum(is_mid)
    ## check that do not have mix of single and lower, upper labels
    ...
    ## extract middle years, if present
    if (has_single)
    ...
    ## extract 'year_mid_min' and 'year_mid_max', if present
    if (has_single) {
    ...
    if (has_low_up) {
        ...
        if (widths_equal)
            width <- widths[[1L]]
        if (has_open_first && widths_equal)
            widths_equal <- ((year_mid_min - year_open_first) %% width) == 0L
    }
