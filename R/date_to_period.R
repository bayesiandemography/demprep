...
date_to_period_or_cohort_year <- function(date,
    ...
    date <- err_tdy_date(x = date,
                         name = "date")
    year_min <- err_tdy_integer_scalar(x = year_min,
                                                name = "year_min")
    year_max <- err_tdy_integer_scalar(x = year_min,
                                                name = "year_max")
    ...
    err_is_logical_flag(x = as_factor,
                        name = "as_factor")
    err_is_gt_scalar(x1 = year_max,
                     x2 = year_min,
                     name1 = "year_max",
                     name2 = "year_min")
    err_lt_year_min(date = date, ## make shortcut for -Inf
                    year_min = year_min,
                    first_month = first_month)
    err_ge_year_max(date = date, ## make shortcut for Inf
                    year_max = year_max,
                    first_month = first_month)
...
chk_lt_year_min <- function(date,
    ...
    if (i > 0L) {
        return(gettextf(paste("'%s' has value [%s] that is less than the",
                        ...
                        name_date, date[[i]], "year_min", "first_month", date_min))
    }
    TRUE
}


chk_ge_year_max <- function(date,
                            year_max,
                            first_month) {
    date_max <- sprintf("%d-%s-01", year_max, first_month)
    date_max <- as.Date(date_max, format = "%Y-%b-%d")
    ge_date_max <- !is.na(date) & (date >= date_min)
    i <- match(TRUE, ge_date_min, nomatch = 0L)
    if (i > 0L) {
        return(gettextf(paste("'%s' has value [%s] that is greater than or equal to the",
                              "maximum date implied  by '%s' and '%s' [%s]"),
                        name_date, date[[i]], "year_max", "first_month", date_max))
    }
    TRUE
}
