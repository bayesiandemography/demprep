...
CONST_P_SINGLE_MONTH <- paste(sprintf("^[0-9]+ %s$", base::month.abb), collapse = "|")

CONST_P_OPEN_FIRST_MONTH <- paste(sprintf("^<[0-9]+ %s$", base::month.abb), collapse = "|")

CONST_P_OPEN_LAST_MONTH <- paste(sprintf("^[0-9]+ %s\\+$", base::month.abb), collapse = "|")
