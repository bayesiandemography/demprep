...
Most demographic datasets, including datasets used in the **dem** packages, are tabulations of people or events classified by categories such as period or age group. The cross-tabulation below is a simple example:
...
kable(deaths_array,
      align = "c",
      caption = "Cross-tabulation of deaths by age and period")
```
The raw data from which these sorts of tables are assembled typically refer to individuals, and give precise dates, rather than periods or age groups. The data frame below is an example:
...
deaths_list <- data.frame(name = c("Aadya",
                          ...
                          date_of_death = c("2020-02-01",
                                            ...
                                            "2020-10-06"))
kable(deaths_list,
      caption = "Individual-level data on date of birth and date of death")
```

Converting dates into periods and age groups can be harder than it looks. Consider, for instance, the problem of measuring a person's age in months. If a person is born on 31 January 2001, on what date does that person become one month old? On 28 February 2001, 1 March 2001, or some other date? According to base **R**, the answer is 3 March 2001:
```{r one-month}
...
Package **demprep** contains a suite of functions for converting dates to periods, age groups, and other categories needed by the **dem** packages. To help resolve cases where the answers are not obvious, such as when deciding when a person turns one month old, **demprep** uses some basic rules that try to reflect common practices in applied demography.
...
When reasoning about age and time, demographers often make use of Lexis diagrams. The figure below is an example of a Lexis diagram. The horizontal axis measures calendar time, and the vertical axis measures age. The black diagonal line marked "A" is the "life line" for a person who was born on `r strftime(dob[1], "%e %B %Y")` and who died on `r strftime(date[1], "%e %B %Y")`. Similarly, the line marked "B" is the life line for a person who was born on `r strftime(dob[2], "%e %B %Y")` and died on `r strftime(date[2], "%e %B %Y")`. A life line in a traditional Lexis diagram always covers the same distance along the vertical axis as it does along the horizontal axis, because a person always gains one year of age for each year of calendar time.
...
The light blue grid in the Lexis diagram above depicts an "age-time plan" [references]. An age-time plan is a set of rules for how points on the Lexis plane are assigned to age and time categories. The age-time plan above divides divides age into 5-year age groups, and divides time into 5-year periods, each starting on 1 January. The deaths of A and B in the Lexis diagram above both belong to the age group "15-19" and to the period "2020-2025".

Although age group and period are the most common age-time categories in demography, the idea of the "cohort" is also important. A cohort is a group of people who all experienced the same defining event during the same period. The defining event is normally birth, but it can be, for instance, marriage or entering a organization. The birth cohort of 1950-1960, for instance, is everyone born during the 1960s, and the marriage cohort of 2018 is everyone who married during 2018.

On a Lexis diagram, the birth cohort for a person or event can be inferred by following the relevant life line back to the horizontal axis and noting the period that the birth falls within. In the Lexis diagram above, for instance, A belongs to the birth cohort "2000-2005" and B belongs to the birth cohort "2005-2010".

When working with cohorts, it is often helpful to add one further category: the Lexis triangle. 


In practice, difficult because of unequal lengths.
