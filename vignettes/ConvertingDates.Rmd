...
Classic demographic datasets, including the demographic arrays used throughout the **dem** packages, use intervals such as periods and age groups to classify events. In the dataset below, for instance, counts of deaths are classified by age and year:
...
kable(deaths_array)
```
Raw data, such as data on a census file or in an administrative database, typically refers to individual people ore events, and uses dates rather than periods or age groups. The dataset below, for instance, contains one line per person, and shows the date of the person's birth and death:

```{r deaths-list, echo = FALSE}
deaths_list <- data.frame(names = c("Aadya",
                                    "Diya", 
                                    "Saanvi", 
                                    "Amaira"),
...
Converting individual-level dates into intervals such as periods and age groups can be harder than it first appears. Consider, for instance, the problem of measuring a person's age in completed months. If a person is born on 31 January 2001, one what day does that person turn one month old: 28 February 2001, 1 March 2001, or some other date? According to base **R**, the answer is 3 March 2001:
```{r one-month}
...
Package **demprep** contains a suite of functions to help with converting dates to intervals. These functions have a consistent format, and follow common principles. In some cases, such as deciding when a person turns one month old, there is no universally agreed-upon answer. In such cases, **demprep** favours answers that meet the needs of applied demography.

ROADMAP FOR VIGNETTE.
...
library(demprep)
date <- as.Date(c("2020-12-22", "2022-05-14"))
dob <- as.Date(c("2001-07-22", "2006-03-14"))
age <- c(19 + 5/12, 16 + 2/12)
```
