...
There are, however, a few common data preparation tasks that do not fit neatly into the workflow above, that can be done using **demprep** functions:
...
**Rotating age, period, cohort, and triangle labels**
...
Like functions `date_to_period_year` and `date_to_cohort_year`, function `date_to_triangle_year`  has a `month_start` argument, which shifts the start date of the triangles.
```{r}
deaths %>%
  mutate(triangle = date_to_triangle_year(date = date_death,
	                                        dob = date_birth,
...
Most published demographic data provides age groups and periods, rather than precise dates. Preparing published demographic data for use typically requires cleaning up existing age group and period labels, rather than calculating new ones from scratch. **demprep** provides the following functions for cleaning up existing labels:
...
Functions `clean_cohort` and `clean_period` are identical to each other, except that `clean_cohort` accepts intervals that are open on the left, while `clean_period` does not.
...
To see how `clean_age`, `clean_period`, and `clean_cohort` have interpreted some labels, use functions`clean_age_df`, `clean_period_df`, and `clean_cohort_df`.
...
To check whether `clean_age`, `clean_period`, and `clean_cohort` have been able to parse the entire vector, use functions `is_valid_age`, `is_valid_period`, and `is_valid_cohort` do.
...
To catch problematic inputs, some version of
```{r, eval = FALSE}
...
The `date_to` and `clean` functions typically do not  


age, period, cohort, and triangle labels may not yet be in the 
