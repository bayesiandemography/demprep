...
We sometimes need to convert single-year period or cohort labels from a calendar-year-at-start format to a calendar-year-at-end format, or vice versa. Because the labeling conventions are confusing, it is easy to get the conversion wrong. To avoid errors, it is safest to use functions `flip_to_start` and `flip_to_end`.
```{r}
x <- c("2001", "2006", "2013")
flip_to_end(x, 
	    month_start = "Apr")
...
```{r}
deaths <- deaths %>%
  mutate(year = flip_to_start(year))
unique(deaths$year)
```
