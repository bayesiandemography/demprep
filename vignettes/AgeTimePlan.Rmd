...
- Raw individual-level data uses dates - birth, occurrence, measurement - continuous
- Demographic analyses use intervals - periods, age, cohort, triangle - discrete
...
labels_age <- make_labels_age(breaks = breaks_age,
                              open_last = FALSE)
demprep:::plot_date_to_age_triangle(date = as.Date("2023-04-18"),
                                    dob = as.Date("2001-07-22"),
                                    unit = "year",
...
```{r lexis2, echo = FALSE, fig.width = 5, fig.height = 5}
breaks_time <- seq.Date(from = as.Date("1995-01-01"), 
                        to = as.Date("2025-01-01"), 
                        by = "10 years")
breaks_age <- seq.int(from = 0, 
                      to = 30, 
...
| Units | Periods | Cohorts | Age groups | Lexis triangles |
|:------|:--------|:--------|:-----------|:----------------|
| years | `date_to_period_year` | `date_to_cohort_year` | `date_to_age_year` | `date_to_triangle_year` |
| years | `date_to_period_multi` | `date_to_cohort_multi` | `date_to_age_multi` | `date_to_triangle_multi` |
| years | `date_to_period_custom` | `date_to_cohort_custom` | `date_to_age_custom` | \<none\> |
| years | \<none\> | \<none\> | `date_to_age_lifetab`| \<none\> |
| years | \<none\> | \<none\> | `date_to_age_births` | `date_to_triangle_births` |
| quarters | `date_to_period_quarter` | `date_to_cohort_quarter` | `date_to_age_quarter` | `date_to_triangle_quarter` |
| months | `date_to_period_month` | `date_to_cohort_month` | `date_to_age_month` | `date_to_triangle_month` |
