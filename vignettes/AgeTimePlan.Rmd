...
```{r}
births <- data.frame(date = c("2020-05-18", "2020-07-02", "2020-10-05"),
                     dob = c("2000-02-14", "2002-05-18", "1990-12-18"))
births$age <- date_to_age_births(date = births$date,
                                 dob = births$dob)
births
```
...
plot_date_to_age_births(date = births$date,
                        dob = births$dob)
```

```{r}
births_outside <- data.frame(date = c("2020-03-14", "2020-08-22"),
                             dob = c("1968-01-23", "2008-03-05"))
births_outside$age <- date_to_age_births(date = births_outside$date,
                                         dob = births_outside$dob,
                                         break_min = 10,
                                         break_max = 55)
births_outside
```
...
plot_date_to_age_births(date = births_outside$date,
                        dob = births_outside$dob,
                        break_min = 10,
                        break_max = 55)
```
...
```{r}
births_outside$age_recoded <- date_to_age_births(date = births_outside$date,
                                                 dob = births_outside$dob,
                                                 recode_up = TRUE,
                                                 recode_down = TRUE)
births
...
3. Use date at start of period and dob to calculate age group at start of period
4. If age group now is equal to age group at start of period, then event occurred in Upper triangle; otherwise occurred in Lower triangle.
