...
test_that("dtabs_survey gives similar but not identical answers when FUN is mean and median", {
    for (seed in 1:5) {
        set.seed(seed) 
        d <- data.frame(y = rbinom(n = 20, size = 1, prob = 0.25),
                        f1 = rep(1:2, times = 10),
        ...
        ans_mean <- dtabs_survey(d, y ~ f1 + f2, weights = wt, FUN = mean)
