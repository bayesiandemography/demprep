...
test_that("dtabs throws correct error with invalid inputs", {
    d <- data.frame(y = rep(c(0, 0, 1, 0), times = 5),
                    f1 = rep(1:10, times = 2),
                    f2 = rep(c("a", "b"), each = 10))
    expect_error(dtabs_survey(y ~ f1 + f2, d),
                 "first argument is a formula : should be a data frame")
})
...
test_that("dtabs_survey gives same answer as xtabs when the weights are uniform", {
    d <- data.frame(y = rep(c(0, 0, 1, 0), times = 5),
                    ...
                    f2 = rep(c("a", "b"), each = 10),
                    wt = rep(2, times = 20))
    ans_obtained <- dtabs_survey(d, y ~ f1 + f2, wt = wt)
...
test_that("dtabs_survey behaves correctly when response has NA", {
    d <- data.frame(y = rep(c(0, 0, 1, 0), times = 5),
                    ...
                    wt = rep(1:5, times = 4))
    d$y[1] <- NA
    ans_obtained <- dtabs_survey(d, y ~ f1 + f2, wt = wt) # na_rm is FALSE
    expect_true(any(is.na(ans_obtained$successes)))
    expect_true(!any(is.na(ans_obtained$trials)))
    ans_obtained <- dtabs_survey(d, y ~ f1 + f2, wt = wt, na_rm = TRUE)
    expect_true(!any(is.na(ans_obtained$successes)))
    expect_true(!any(is.na(ans_obtained$trials)))
})
...
test_that("dtabs_survey gives same answer with logical and 0-1", {
    for (seed in 1:5) {
        ...
        d_01 <- d_logical
        d_01$y <- d_01$y * 1L
        ans_logical <- dtabs_survey(d_logical, y ~ f1 + f2, wt = wt)
        ans_01 <- dtabs_survey(d_01, y ~ f1 + f2, wt = wt)
...
test_that("dtabs_survey throws correct error with invalid inputs", {
    d <- data.frame(y = rep(c(0, 0, 1, 0), times = 5),
                    ...
                    f2 = rep(c("a", "b"), each = 10),
                    wt = rep(1:2, times = 10))
    expect_error(dtabs_survey(d, y ~ f1 + f2, wt = wt),
                 "no cells have 2 or more observations, so no design effects can be calculated")
    expect_error(dtabs_survey(y ~ f1 + f2, d, wt = wt),
                 "first argument is a formula : should be a data frame")
})
